...
#%%
from ipyleaflet import Map, basemap_to_tiles, basemaps, DrawControl, WidgetControl, LayersControl
...
class InteractiveMap:
    ...
    def __init__(self, center = (40.41671327747509, -3.702635610085826), zoom = 0):
        ...
        self.draw_control = DrawControl(rectangle={"shapeOptions": {"color": "#f06eaa"}})
    ...
    def handle_draw(self, target, action, geo_json):
        if action == "created":
            ...
            aoi_last_coordinates = [[[aoi_lons.min(), aoi_lats.min()],
    ...
    def get_OSM_tileRange(self):
        ...
        if len(self.aoi_coordinates)==0:
            ...
            c1, c2, c3, c4, = self.aoi_extent_coordinates[-1][-1]
            ...
            ytile_1 = int(n * (1 - (np.log(np.tan(np.deg2rad(lat_deg_1)) + 1/np.cos(np.deg2rad(lat_deg_1))) / np.pi)) / 2)
            xtile_2 = int(n * ((lon_deg_2 + 180) / 360))
            ytile_2 = int(n * (1 - (np.log(np.tan(np.deg2rad(lat_deg_2)) + 1/np.cos(np.deg2rad(lat_deg_2))) / np.pi)) / 2)
            xtile_3 = int(n * ((lon_deg_3 + 180) / 360))
            ytile_3 = int(n * (1 - (np.log(np.tan(np.deg2rad(lat_deg_3)) + 1/np.cos(np.deg2rad(lat_deg_3))) / np.pi)) / 2)
            xtile_4 = int(n * ((lon_deg_4 + 180) / 360))
            ytile_4 = int(n * (1 - (np.log(np.tan(np.deg2rad(lat_deg_4)) + 1/np.cos(np.deg2rad(lat_deg_4))) / np.pi)) / 2)
            ...
            corner_tiles = np.array([[xtile_1,ytile_1],
                            [xtile_2,ytile_2],
                            [xtile_3,ytile_3],
                            [xtile_4,ytile_4]])
            
            range_tiles = [[min(corner_tiles[:,0]),min(corner_tiles[:,1])],
    ...
    def download_OSM_buildings(self,show_tiles=False):
        ...
        for xtile in tqdm(np.arange(xtile_1,xtile_4+1)):
            for ytile in tqdm(np.arange(ytile_1,ytile_4+1), leave=False):
                ...
                querystring = {"limit":"2","divider":"3","zoom":"6","lat_ne":"36.5979","lon_ne":"28.1250","lat_sw":"34.3071","lon_sw":"25.3125","date_end":"last","quality":"7"}
                payload = ""
                headers = {
    ...
    def merge_tiles(self):
        ...
        # save merge
        schema = {"id":"int","geometry": "Polygon", "properties": {"id":"int",
                                                                   "height":"int",
                                                                   #"levels":"int",
                                                                   "perimeter":"int",
                                                                   "planar_area":"int",}}
        
        with fiona.open(self.merge_file_name, "w", driver="GeoJSON", crs="EPSG:4326", schema=schema) as f:
    ...
    def add_geo_em_file(self,nc_file_path,add_to_map=True):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print("No variable named: (XLONG/XLONG_C/XLONG_M) or (XLAT/XLAT_C/XLAT_M)")
        ...
        self.geo_em_file_extent_coordinates.append([[lon_min, lat_min],
                                        [lon_min, lat_max],
                                        [lon_max, lat_max],
                                        [lon_max, lat_min],
                                        ])
        ...
        json_data_base['features'][0]['geometry']['coordinates'] = self.geo_em_file_extent_coordinates
        self.geo_em_file_extent_data.append(json_data_base)
        self.geo_em_file_extent_geoms.append({"coordinates": self.geo_em_file_extent_coordinates, "type": "Polygon"})
        #self.geo_em_file_extent_shape.append(shapely.geometry.shape(self.aoi_geoms[-1]))
        self.geo_em_file_extent_shape.append(shapely.geometry.shape(self.geo_em_file_extent_geoms[-1]))
