...
class InteractiveMap:
    ...
    def calculate_URB_PARAM(self, nc_file_path=None, nc_file_path_to_save=None, json_data_path=None, save_temp_files=False):
        ...
        def merge_overlapping_polygons(polygons,delta_lat=9e-6,delta_lon = 1e-5):
            ...
            for id_i in tqdm(range(id_i_max)):
                for id_j in range(id_j_max):
                    ...
                    # Take the subset of the ones in proximity by gridcell
        ...
        # Open the NetCDF file
        if nc_file_path:
            ncfile = xr.load_dataset(nc_file_path)
        ...
        if json_data_path:
            ...
            with open(json_data_path) as f:
                data = json.load(f)
                self.combined_rows = data['features']
        ...
        # Change. Pass it to dataframe first and assign gridcell. like that it is faster to run the merge. 
        ...
        # Extract polygons and their attributes
        polygons = [(shape(feature['geometry']), feature['properties'], feature['id_i'], feature['id_j']) for feature in data['features']]
        ...
        print('Calculating properties of polygons...')
        # Calculate the ratio from haversine
